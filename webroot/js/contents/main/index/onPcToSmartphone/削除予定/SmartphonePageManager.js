...
jQuery(document).ready(function($){
	 ...
	 * ライブラリエレメント管理クラス
	 */
	MYNAMESPACE.namespace('modules.main.index.onPcToSmartphone.SmartphonePageManager');
	MYNAMESPACE.modules.main.index.onPcToSmartphone.SmartphonePageManager = function() {
		this.initialize.apply(this, arguments);
	};
	MYNAMESPACE.modules.main.index.onPcToSmartphone.SmartphonePageManager.prototype = $.extend({}, MYNAMESPACE.modules.main.index.onPcToSmartphone.BaseRightElementManager.prototype, {
		_isEnabled						: false
		...
		,setElement: function(data, id) {
			...
			for (var i=0,len=data.length; i<len; i++) {
				var forTemplateData = {
					'pageId'				: data[i]['smartphone_pages']['id'],
					'pageTitle'				: data[i]['smartphone_pages']['title'],
					'pagePath'				: data[i]['smartphone_pages']['path'],
					'pageUrl'				: pageUrl,
					'editer'				: data[i]['editedUser']['name'] ? data[i]['editedUser']['name'] : '-----',
					'edited'				: data[i]['smartphone_pages']['edited'] ? data[i]['smartphone_pages']['edited'] : '-----',
					'publisher'				: data[i]['publishedUser']['name'] ? data[i]['publishedUser']['name'] : '-----',
					'published'				: data[i]['smartphone_pages']['published'] ? data[i]['smartphone_pages']['published'] : '-----',
					'isLatest'				: data[i]['smartphone_pages']['is_latest_page']
				}
				$targetElem.find('> ul.page-list').append($(_.template($('#smartphone-page-item-template').text(), forTemplateData)));
			}
		}
