...
//$(function() {
	 ...
	 */
	MYNAMESPACE.namespace('modules.library.catalog.onPcToSmartphone.DataManager');
	MYNAMESPACE.modules.library.catalog.onPcToSmartphone.DataManager = function() {
		this.initialize.apply(this, arguments);
	};
	MYNAMESPACE.modules.library.catalog.onPcToSmartphone.DataManager.prototype = {
		_isEnabled						: false
		,_prop							: {}
		,_directoryId					: []
		,historyOfDirectoryIndex		: 2
		,_data							: {}
		,_instances						: {}
		,_isAccess						: false
		...
		/*
		 * カレントなディレクトリIDをセットするメソッド
		 * @param	id			カレントなディレクトリID
		 * @return	void
		 */
		,setDirectoryId: function(id) {
			var thisObj = this;
			if (id && thisObj._directoryId[0] !== id) {
				thisObj._directoryId.unshift(id);
				if (thisObj.historyOfDirectoryIndex <= thisObj._directoryId.length) {
					thisObj._directoryId.pop();
				}
				$(thisObj).trigger('onChangeCurrentDirectoryId', thisObj._directoryId[0])
			}
		}

		/*
		 * ディレクトリIDを返すメソッド
		 * @param	index			0 or undefinedでカレントなディレクトリID、1以上の値でindex個前にカレントだったディレクトリID
		 * @return	ディレクトリID
		 */
		,getDirectoryId: function(index) {
			var thisObj = this;
			index = index === undefined ? 0 : index;
			iindex = thisObj._directoryId.length <= index ? thisObj._directoryId.length - 1 : index;

			return thisObj._directoryId[index] ? thisObj._directoryId[index] : -1;
		}
		...
		,request: function() {
			...
			var url = thisObj._prop['currentUrl'] + 'Library/getLibraryList/';
			var sendData = {
