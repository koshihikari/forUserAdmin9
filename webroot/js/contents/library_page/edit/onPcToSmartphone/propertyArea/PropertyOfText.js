...
//$(function() {
	 ...
	 * eventName
	 */
	MYNAMESPACE.namespace('modules.PropertyOfText');
	MYNAMESPACE.modules.PropertyOfText = function() {
		this.initialize.apply(this, arguments);
	};
	MYNAMESPACE.modules.PropertyOfText.prototype = {
		_isEnabled						: false
		,_elementType					: ''
		,_eventArr						: []
		...
		,initialize: function(instances, id) {
			...
			// _.bindAll(this, 'getElem', 'getProperty', 'createElement', 'refreshStyle', 'onChangeColor', 'onChangeSlider', 'onChangeText', 'onCheckHandler', 'onChangeTextarea', 'setEvent');
			_.bindAll(
				this
				,'getElem'
				,'getProperty'
			...
			this._currentVal['text'] = this._currentVal['text'].replace(/<br \/>|<br>/g, "\n");
		...
		,createElement: function(id) {
			...
			var source = '\
						<div class="propertyElement forTextElement">\
							<p class="elementTitle">テキスト</p>\
							<div class="contentWrapper">\
								<div class="textInputWrapper">\
									<div class="control-group">\
										<label class="control-label">文言</label>\
										<div class="btn-hole">\
											<div class="btn-group" data-toggle="buttons-checkbox">\
												<button type="button" class="btn btn-primary isEnabledHtml' + (thisObj._currentVal['isEnabledHtml'] === true ? ' active' : '') + '" data-item-name="isEnabledHtml" data-placement="left" data-original-title="' + thisObj.tooltipMessages['isEnabledHtml'][0] + '"><span><i data-icon=""></i>HTMLを有効にする</span></button>\
											</div>\
										</div>\
									</div>\
									<textarea>' + thisObj._currentVal['text'] + '</textarea>\
								</div>\
		...
		,onChangeTextarea: function(event) {
			...
			if (thisObj._currentVal['text'] !== val) {
				var targetElem = $("#" + thisObj._targetId);
				if (val !== "") {
			...
			if (isEnabled === true && thisObj._isEnabled === false) {
				...
				thisObj._elems['propertyElement']
					.find(".textInputWrapper textarea")
						.on('change', function(event) {
							thisObj.onChangeTextarea(event);
						})
						.on('focus', function(event) {
							if (thisObj._timerId) {
								clearInterval(thisObj._timerId);
							}
							thisObj._timerId = setInterval(function(event2) {thisObj.onChangeTextarea(event);}, 100);
						})
						.on('blur', function(event) {
							if (thisObj._timerId) {
								clearInterval(thisObj._timerId);
