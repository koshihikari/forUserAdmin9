...
jQuery(document).ready(function($){
	...
	MYNAMESPACE.modules.library_page.catalog.onPcToSmartphone.PageManager.prototype = {
		...
		,_updateData					: {}
		,_hasSaveData					: {}
		,_notLatestMessage				: 'このページは書きだした後に編集されています。<br />最新版ではない可能性があります。'
		...
		,refresh: function(data) {
			var thisObj = this;
			var taregetElement = $('#record-id_' + data['id']);
			taregetElement
				.find('input [name="title"]').val(data['title'])
			.end()
				.find('input [name="path"]').val(data['path'])
			.end()
				.find('.editor').text(data['edited_ueser_name'])
			.end()
				.find('.edited').text(data['edited'])
			.end()
				.find('.publisher').text(data['published_user_name'])
			.end()
				.find('.published').text(data['published']);

			if (data['is_latest_page'] === true) {
				taregetElement
