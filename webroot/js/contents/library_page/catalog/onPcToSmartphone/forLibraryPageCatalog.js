...
jQuery(document).ready(function($){
	...
	$(ConcretePageManager)
		.on('onCompleteStaticPagePublish', function(event, data) {
			// console.log('data');
			// console.log(data);
			...
			// 	Util.showCurtain('ページ書き出し開始(ページID :: ' + bulkPublishData['idArr'][0] + ')', 300);
			// 	ConcretePageManager.publish(bulkPublishData['idArr'][0], true);
			// } else {
			// 	Util.hideCurtain();
			// }
		});
	...
	$(PageManager)
		...
		.on('onClickBulkPublishBtn', function(event, recordIdArr) {
			bulkPublishData = {
				'idArr'			: recordIdArr,
				// 'total'			: recordIdArr.length,
				'count'			: 0
			};
	...
	function bulkPublish() {
		...
		// if (0 < bulkPublishData['idArr'].length) {
			...
			ConcretePageManager.publish(bulkPublishData['idArr'][bulkPublishData['count']], true);
	...
	DataManager.request();
		...
		$('#packages a').click(function (e) {
			e.preventDefault();
			$(this).tab('show');
			console.log(0)
			return false;
		})
		$('#packages a:last').tab('show');
});
