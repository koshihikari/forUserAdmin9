...
class Library_Smartphone_UpdateElementAction extends AppModel {
		...
		if ($request->isPost()) {
			if (
				...
				if ($is_correct === true) {
					...
					$this->rollback();
					$json = array('result'=>false, 'message'=>'データ更新に失敗しました。');
				}
			}
			...
			// 	$this->begin();
			// 	$isCorrect = true;
			// 	$isCorrect = $this->_updateElementData($isCorrect, $request);
			// 	if ($isCorrect === true) {
			// 		$this->commit();
	...
	private function _updateElementData($is_correct, $request) {
		if ($is_correct === true) {
			...
			if ($request->data['update_data']['id'] !== '' && $request->data['update_data']['id'] !== '0') {
				...
				// $data['id'] = $request->data['update_data']['id'];
				return is_array($this->save($data));

			} else if ($request->data['update_data']['element_id'] !== '') {
				$data['smartphone_library_id'] = $request->data['base_data']['smartphone_library_id'];
	 ...
	 * @return	boolean
	 */
	// private function _updateElementData($isCorrect, $request) {
	// 	if ($isCorrect === true) {
	// 		$data = array(
