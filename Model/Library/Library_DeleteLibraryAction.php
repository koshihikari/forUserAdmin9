...
class Library_DeleteLibraryAction extends AppModel {
	...
	public function deleteLibrary($request) {
		...
		if ($request->isPost()) {
			...
			if (
				...
				// error_log($result['PageDeleteAction']['order'] . "\n", 3, 'log.txtLibrary_DeleteLibraryAction				$result2 = $this->find('all', array('conditions'=>array('residence_id'=>$request->data['residence_id'], 'order >'=>$result['Page_DeletePageAction']['order'])));
				$result2 = $this->find('all', array('conditions'=>array('residence_id'=>$request->data['residence_id'], 'order >'=>$result['Library_DeleteLibraryAction']['order'])));
				for ($i=0,$len=count($result2); $i<$len; $i++) {
					// error_log('	' . $i . ', id = ' . $result2[$i]['PageDeleteAction']['id'] . "\n", 3, 'log.txt');
					$updateData = array(
						'id'	=> $result2[$i]['Library_DeleteLibraryAction']['id'],
						'order'	=> $result2[$i]['Library_DeleteLibraryAction']['order'] - 1
					);
