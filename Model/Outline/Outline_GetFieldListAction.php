	...
	public function getFieldList($request) {
		...
		if ($request->isPost()) {
			...
			if (
				...
				$result = ClassRegistry::init('Outline_GetOutlineAction')->getOutline($request->data['residence_id'], $request->data['user_id']);
				$json = array('result'=>true, 'data'=>$result['outlines']);
