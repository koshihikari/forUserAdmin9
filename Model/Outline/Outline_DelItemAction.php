...
class Outline_DelItemAction extends Model {
	 ...
	 * @return	json
	 */
	public function delItem($request) {
		...
		if ($request->isPost()) {
			if (
				...
				$this->begin();
				$is_correct = true;
				$is_correct = $this->_delItem($is_correct, $request);
	 ...
	 * @return	boolean
	 */
	private function _delItem($is_correct, $request) {
		if ($is_correct === true) {
			$conditions = array(
				'id'			=> $request->data['update_data']['id']
			);
	...
	private function _reorderItem($is_correct, $request) {
		if ($is_correct === true) {
			...
			for ($i=0,$len=count($result); $i<$len; $i++) {
				$data = array(
					'id'			=> $result[$i]['Outline_DelItemAction']['id'],
					'order'			=> $i + 1
				);
